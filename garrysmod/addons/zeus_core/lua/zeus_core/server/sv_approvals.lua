...
function Approvals.CreateTransfer(staff, target, toRegiment)
    ensureTable()
    ...
    sql.Query(string.format([[
        ...
        sql.SQLStr(toReg or ""),
        sql.SQLStr(target.zeusData.rank or ""),
        sql.SQLStr(target.zeusData.rank or ""), -- no rank change on transfer by default
...
function Approvals.Resolve(ply, id, accept)
    ...
    if accept and row.type == "transfer" then
        ...
        if IsValid(target) and ZEUS.Identity and ZEUS.Identity.AssignToRegiment then
            target.zeusData = target.zeusData or {}
            -- Keep existing rank on transfer
            local ok, err = ZEUS.Identity.AssignToRegiment(ply, target, row.to_regiment, target.zeusData.rank or "PVT")
            if not ok then
